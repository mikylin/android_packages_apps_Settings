...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("kylin_lock")) {
            resId = R.string.lockscreen_lock_icon_kylin;
    ...
    private void resizeKylinLock() {
        Bitmap kylinLock = BitmapFactory.decodeResource(getResources(), R.drawable.kylin_lock);
        if (kylinLock != null) {
            String path = null;
            int px = requestImageSize();
            kylinLock = Bitmap.createScaledBitmap(kylinLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "kylin_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                kylinLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
